...
func (sched *ExampleScheduler) ResourceOffers(driver sched.SchedulerDriver, offers []*mesos.Offer) {
	...
	for _, offer := range offers {
		...
		MEM_PER_TASK <= remainingMems {
			...
			if err != nil {
				log.Errorf("Failed serializing the job %d", job.ID)
				log.Error(err)
				panic(err)
			}
...
func (sched *ExampleScheduler) StatusUpdate(driver sched.SchedulerDriver, status *mesos.TaskStatus) {
	...
	ctjob,err := lq.DeserializeJob(status.GetData())
	if (err != nil){
		log.Error("Could not get desearlize job ")
	}

	log.Error(ctjob)
	log.Errorln("Got Container ID ::::: ", ctjob.ContainerId)
	...
	if sched.tasksFinished >= sched.totalTasks {
		log.Infoln("Total t asks completed, stopping framework.")
		//driver.Stop(false)
	}
